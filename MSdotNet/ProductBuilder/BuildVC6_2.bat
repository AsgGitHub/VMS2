@echo --------------------------------------------------------------------------
@echo BuildVC6_2.bat - Build the MSVC++ 6 components
@echo --------------------------------------------------------------------------
@setlocal
@pushd .
@echo on
...
@echo set up logfiles
  ...
  @copy nul %CurrentPassLogFile% > nul
  @set LastPassLogFile=q:\build_temp\msvc\BuildVC6_last_pass.log

@echo Generate Build Compilation List
  @%PBToolsPath%\ResolveComplist q:\ /o:%msvc_temp_dir%\complist.txt /d:mak > nul
  ...
  @copy /b %msvc_temp_dir%\complist.txt %msvc_temp_dir%\complist_relink_unordered.txt > nul
  
@if /i "%RelinkVC6%"=="false" @goto NORELINK
@echo Generate Relink Compilation List
  @if exist %msvc_temp_dir%\complist_relink.txt @del /f /q %msvc_temp_dir%\complist_relink.txt
  @%PBToolsPath%\ResolveComplist q:\ /a /o:%msvc_temp_dir%\complist_relink_unordered.txt /d:mak > nul
  @if errorlevel 2 @set BuildVC6Error=true & @goto ENDCOMPILATION
:NORELINK
  
@echo Generate Ordered Relink Complist -- add hint files
  @set RelinkComplist=%msvc_temp_dir%\complist_relink.txt
  @for /F %%o in (q:\txt\vc_complist_order.txt) do @for /F %%u in (%msvc_temp_dir%\complist_relink_unordered.txt) do @if /i "%%o"=="%%u" @echo %%u>>%RelinkComplist%
@echo Generate Ordered Relink Complist -- add remaining files
  @for /F %%u in (%msvc_temp_dir%\complist_relink_unordered.txt) do @findstr /i /x "%%u" %RelinkComplist%>nul || @echo %%u>>%RelinkComplist%
@echo.
...
@echo.
@echo Clean out all the intermediate and output files for the MSVC components ...
@echo.
...
@echo copy bootstrap lib files from managed locations
  @if exist q:\vendors\mobiusbootstrap\  xcopy /r /i /y q:\vendors\mobiusbootstrap\*  q:\lib\
